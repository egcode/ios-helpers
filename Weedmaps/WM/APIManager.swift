...
class APIManager {
    ...
    class func get(url: String, token: String, response: @escaping Response) {
        self.request(method: .get, url: url, token: token) { (object, code, error) in
            response(code, object)
        }
    }
    
    private class func request(method: Method, url: String, payload: Any? = nil, token: String, completionHandler: @escaping (Any?, Int, Error?) -> Swift.Void) {
        ...
        request.setValue(token, forHTTPHeaderField: "Authorization") // Using our Client-ID in header
    ...
    class func getApiUrl(term: String, location: String) -> String {
        return "https://api.yelp.com/v3/businesses/search?term=\(term)&location=\(location)".lowercased()
    }
}
